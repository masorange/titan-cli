...
def test_gemini_base_url_normalization(mock_gemini_provider_config, mocker):
...
def test_gemini_custom_endpoint_usage(mock_gemini_provider_config, mock_requests_post):
...
def test_gemini_official_endpoint_usage(mock_gemini_provider_config, mocker):
    """Test that the Gemini client uses the official endpoint by default (no custom base_url)."""
    # Mock genai.Client
    mock_genai_client_instance = MagicMock()
    mock_genai_client_instance.models.generate_content.return_value = MagicMock(text="Mocked Gemini Response Text")
    mock_genai_client_instance.models.GenerativeModel.return_value = MagicMock()
    mock_genai_client_instance.chats.create.return_value.send_message.return_value = MagicMock(text="Mocked Chat Response Text")

    # Mock the Client class itself (not google.genai.client.Client)
    mock_genai_client_class = mocker.patch("google.genai.Client", return_value=mock_genai_client_instance)

    # Mock google.auth.default as it's called in the OAuth branch of GeminiProvider.__init__
    ...
    # Ensure genai.Client IS called when official endpoint is used
...
def test_gemini_generate_response_with_custom_endpoint(mock_gemini_provider_config, mock_requests_post):
...
def test_gemini_generate_error_handling_google_endpoint(mock_gemini_provider_config, mocker):
    """Test error handling during response generation for Google endpoint."""
    # Mock genai.Client and its methods
    mock_genai_client_instance = MagicMock()
    # Mock GenerativeModel to return a mock model
    mock_genai_client_instance.models.GenerativeModel.return_value = MagicMock()
    # Configure mock to raise a specific error that GeminiProvider's except block will catch
    mock_genai_client_instance.models.generate_content.side_effect = RuntimeError("Google API is down")

    # Mock the Client class itself
    mocker.patch("google.genai.Client", return_value=mock_genai_client_instance)

    # Mock google.auth.default as it's called in the OAuth branch
    mocker.patch("google.auth.default")
