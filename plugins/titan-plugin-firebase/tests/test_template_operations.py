...
def test_current_value_reports_inheritance(template_payload):
    ...
    value, inherited = current_raw_value(template_payload, "welcome_text", "android_prod")
...
def test_apply_change_touches_only_the_target_value(template_payload):
    ...
    assert updated["parameters"]["welcome_text"] == template_payload["parameters"][
        "welcome_text"
    ]
...
def test_apply_change_does_not_mutate_the_source(template_payload):
    ...
    assert template_payload["parameters"]["feature_enabled"]["defaultValue"][
        "value"
    ] == "false"
