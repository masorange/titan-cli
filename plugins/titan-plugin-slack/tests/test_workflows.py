...
def test_post_file_workflow_structure() -> None:
    workflow_path = (
        Path(__file__).parent.parent / "titan_plugin_slack" / "workflows" / "post-file.yaml"
    )

    with open(workflow_path, encoding="utf-8") as handle:
        workflow = yaml.safe_load(handle)

    assert workflow["name"] == "Post Slack File"
    assert [step["id"] for step in workflow["steps"]] == [
        "validate_connection",
        "select_target",
        "prepare_destination",
    ...
    assert workflow["steps"][1]["step"] == "select_default_or_search_channel_target"
    assert workflow["steps"][4]["step"] == "upload_file"
    assert all("on_error" not in step for step in workflow["steps"])
