...
reading and listing Remote Config templates, comparing keys across projects, creating new
keys, and writing one parameter — in a single project or across several.
...
|------|-------|----------------------------|
| `firebase_auth_check` | Authentication | `create-remoteconfig-key`, `list-projects`, `list-remoteconfig-keys`, `list-remoteconfig-keys-multiproject`, `read-remoteconfig`, `set-remoteconfig-value`, `set-remoteconfig-value-multiproject` |
| `firebase_projects_list` | Project Selection | `list-projects` |
| `firebase_select_target` | Project Selection | `list-remoteconfig-keys`, `read-remoteconfig`, `set-remoteconfig-value` |
| `firebase_select_targets` | Project Selection | `create-remoteconfig-key`, `list-remoteconfig-keys-multiproject`, `set-remoteconfig-value-multiproject` |
| `firebase_remoteconfig_get` | Reading Remote Config | `list-remoteconfig-keys`, `read-remoteconfig`, `set-remoteconfig-value` |
| `firebase_remoteconfig_list_keys` | Reading Remote Config | `list-remoteconfig-keys` |
| `firebase_remoteconfig_fanout_list_keys` | Reading Remote Config | `create-remoteconfig-key`, `list-remoteconfig-keys-multiproject`, `set-remoteconfig-value-multiproject` |
...
| `firebase_remoteconfig_create_key_plan` | Writing Several Projects | `create-remoteconfig-key` |
| `firebase_remoteconfig_create_key_publish` | Writing Several Projects | `create-remoteconfig-key` |
...
| `firebase_remoteconfig_copy_key` | Writing Several Projects | — |
| `firebase_remoteconfig_sync_plan` | Writing Several Projects | — |
| `firebase_remoteconfig_sync_publish` | Writing Several Projects | — |
...
These steps do not hardcode project naming schemes: a project ID is passed in,
configured, or chosen from Firebase Management. A repository can attach its own
generic metadata — labels, brands, environments and groups — and the Firebase
steps carry it forward.

- `firebase_projects_list`: list Firebase projects available to the active ADC session,
  optionally filtering the catalogue by `project_filter`, and publishing both UI models
  and IDs for later steps
- `firebase_select_target`: resolve one project from `project_id` or `default_project`,
  listing available projects in the TUI when neither is configured, applying
  `project_filter`/`firebase_project_filter` when present, falling back to a manual
  Project ID prompt, and recovering optional label, brand and environment metadata
- `firebase_select_targets`: normalize a caller-supplied list — `project_ids` as a workflow
  ...
  `firebase_project_labels`, `firebase_project_brands` and
  `firebase_project_environments`, preserving order and dropping duplicates. When no IDs
  are present, it can resolve a configured `project_set` (optionally filtered by
  `project_groups` and `environment`) before falling back to the TUI project catalogue
...
- `firebase_remoteconfig_list_keys`: list every parameter key in the template, with type
  plus default and condition-specific value summaries
...
Use these steps to create one key or apply one value change to several projects. Every
project has its own template, so each is validated separately and reported separately.

- `firebase_remoteconfig_create_key_plan`: create a typed key request, audit where that key
  is missing, validate the payload per target, reject existing keys and condition values
  that are not common to every selected destination, and confirm project by project
- `firebase_remoteconfig_create_key_publish`: validate and publish planned key creations
  one project at a time, capturing failures so one project's rejection does not lose the
  others
- `firebase_remoteconfig_fanout_plan`: validate the change against every target, show a
  plan table (ready / no change / error), blocking bulk changes when the inventory says
  the selected key is not common with one stable known type or has Firebase-managed value
  sources, and confirm project by project through a multi-select with nothing preselected.
  Selected targets may span several configured environments when the selector included
  them.
...
- `firebase_remoteconfig_copy_key`: choose one key that exists in at least one selected
  project and is missing from another, then plan copies only to missing destinations,
  refusing Firebase-managed source values
- `firebase_remoteconfig_sync_plan`: choose several syncable missing keys and build a
  copy plan for them, skipping Firebase-managed source values, requiring an explicit
  source when a key has several possible origins
- `firebase_remoteconfig_sync_publish`: validate and publish planned key copies one target
  at a time
...
- The multi-project write workflow declares `key`, `value`, `condition`, `project_ids`,
  `project_set`, `project_groups` and `environment` as optional params; an empty value
  means "ask me" or "use the configured default" depending on the step, and the prompts
  are built from the first target's template, filtered to bulk-safe keys when the
  inventory has been read.
- The multi-project key inventory workflow declares `project_ids`, `project_set`,
  `project_groups` and `environment` as optional; an empty project list means "use
  `default_project_set`", "ask me" in the TUI, or "read `firebase_project_ids` from an
  earlier step" when composed by a project-specific workflow.
- `create-remoteconfig-key` creates missing keys only. It never overwrites an existing
  target key, and it refuses condition values unless every selected target declares that
  condition.
- The write workflows edit literal `value` entries and can replace `useInAppDefault`, but
  preserve and refuse to overwrite `personalizationValue`, `experimentValue`,
  `rolloutValue`, or unknown future value-source fields.
- Multi-project write workflows can intentionally target several configured environments
  in one run. The selector asks which environments to include, and publishing remains
  serial and confirmed project by project.
- Type metadata stays Firebase-compatible (`BOOLEAN`, `STRING`, `NUMBER`, `JSON`), while
  tables and prompts use Titan labels (`Bool`, `String`, `Number`, `JSON`) from
  `RemoteConfigValueType`.
- `project_filter` accepts case-insensitive words matched against Firebase project IDs,
  display names, resource names, and project numbers. Use a value such as
  `Prepago, National` to shrink a large catalogue before choosing projects.
...
??? info "`firebase_remoteconfig_list_keys`"
...
??? info "`firebase_remoteconfig_fanout_list_keys`"
...
??? info "`firebase_remoteconfig_create_key_plan`"
...
??? info "`firebase_remoteconfig_create_key_publish`"
...
??? info "`firebase_remoteconfig_copy_key`"
...
??? info "`firebase_remoteconfig_sync_plan`"
...
??? info "`firebase_remoteconfig_sync_publish`"
